# Changes to ScriptFu v3 for script authors

## About

This is only about changes to the ScriptFu  language proper.
It discusses how to edit v2 scripts
so they will run in GIMP 3.

The GIMP PDB, which you can call in scripts, has also changed.
See TODO.  That also may require you to edit scripts.

The installation and behaviour of scripts has also changed.
See TODO
...
### In scripts, calls to PDB procedures returning a boolean return (#t) or (#f)
...
So "yield #f" does not mean a boolean is returned to the caller.
...
It lets you declare a multi-layer capable filter,
that is, taking an image and drawables.
...
must have those arguments.  For example:
...
but that take an image and single drawable,
are deprecated.
...
GIMP enables the menu item for such scripts if and only if a user
...
also called the drawable arity of the algorithm.
...
So a plugin procedure called by GIMP should never receive
